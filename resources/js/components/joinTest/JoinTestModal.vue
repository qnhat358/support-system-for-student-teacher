...
<template>
  <div class="bg-black w-full h-full fixed z-50 top-0 flex justify-center items-center p-2">
      ...
      <div class="relative z-10 mb-6 bg-[var(--second)] w-full p-5 rounded-lg text-white text-bold overflow-hidden"
        ...
        <div class="font-bold flex justify-between gap-2 items-center w-1/2">
          ...
          <select id="questionType" class="w-56 h-7 rounded-md p-0 px-3 bg-[var(--primary)]"
            v-model="questions[currentQuestionIndex].type" disabled>
            <option value="">Choose type</option>
            <option value="multiple choice">Multiple choice</option>
            <option value="enter result">Enter result</option>
          </select>
          <label for="questionType" class="min-w-fit">Points:</label>
          <select id="questionType" class="w-30 h-7 rounded-md p-0 px-3 bg-[var(--primary)]"
            v-model="questions[currentQuestionIndex].point" disabled>
        ...
        <div class="mt-4 grid gap-2 relative" :class="`grid-cols-${questions[currentQuestionIndex].answers.length}`">
        ...
        <div class="flex justify-end gap-4 mt-4 text-xl font-bold">
          <button v-if="currentQuestionIndex < questions.length - 1" class="w-24 p-2 bg-[var(--primary)] rounded-lg"
